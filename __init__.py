...
class ASSET_PT_main(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row = box.row()
        row.prop(assettool, "use_auto_scale")
        box = layout.box()        
        row = box.row()        
        row.scale_y=1.7
...
#                print(os.path.join(root, subdirectory))
            ...
            if has_map:                                
                ...
                if assettool.use_auto_scale:     
                    autoscale_attribute=material_new.node_tree.nodes.new('ShaderNodeAttribute')
                    autoscale_attribute.location=(-1250,-150) 
                    autoscale_attribute.attribute_type='OBJECT'
                    autoscale_attribute.attribute_name="scale"
                    material_new.node_tree.links.new(autoscale_attribute.outputs[1],mapping.inputs[3])
                ...
                for file_name in files:
                    ...
                    if (file_name.find("alpha")!=-1 or file_name.find("opacity")!=-1) and assettool.use_alpha:   
...
class AssetSceneProperties(PropertyGroup):
    ...
    use_auto_scale : BoolProperty(
       name = "Automatically scale texture",
       default = True,
       description='Automatically scale texture according to object scale'
       ) 
